...
Details <- read_csv("/Users/Ishneet Kaur Kataria/Downloads/DsProject-main/IndividualDetails.csv")
...
d_subset$diagnosed_date<-as.Date(d_subset$status_change_date,'%d/%m/%Y')
...
age<-read_csv("/Users/Ishneet Kaur Kataria/Downloads/DsProject-main/AgeDistribution.csv")
age<-as.data.frame(age)
lbls<-c("0-9","10-19","20-29","30-39","40-49","50-59","60-69","70-79",">=80")
pct<-c(age$Percentage)
lbls<-paste(lbls,pct,sep = "->")
pie(age$TotalCases,labels=lbls,col=rainbow(length(age$AgeGroup)),radius=1.2,border=1,main="AgeGroup wise distribution of cases")

status<-table(d_subset$current_status)
status
barplot(status,col=c("green","blue","yellow"),xlab = "Current Status of the patients",ylim=c(0,1000),ylab = "No. of patients",main = "Status of patients")
sum(status)

spect<-round(status/sum(status)*100)
lbls<-c("Deceased","Hospitalized","Recovered")
...
c2
rd<-c("Males","Females")
barplot(c1, main="Male Female recovery distribution",names.arg = rd,
        ylab="no of patients",ylim =c(0,50), col=c("blue","orange"),
         )
barplot(c2, main="Male Female deceased distribution",names.arg = rd,
        ylab="no of patients",ylim =c(0,50), col=c("blue","orange"),
)
...
pie(pct,labels = lbls, col=rainbow(length(lbls)),
    main="Recovery percentage of females vs males")
...
lbls <- paste(lbls,"%",sep="") # ad % to labels
pie(pct,labels = lbls, col=rainbow(length(lbls)),
    main="Deceased percentage of females vs males")
...
lbls <- paste(lbls,"%",sep="") # ad % to labels
pie(pct,labels = lbls, col=rainbow(length(lbls)),
    main="Hospitilized percentage of females vs males")
...
barplot(State$cases,col=rainbow(29),names.arg = State$state,las=2,ylab = "No. of patients",main = "No. of Patients State wise")
...
barplot(nation$cases,col=rainbow(8),names.arg = nation$nationality,las=2,ylab = "No. of patients",main = "No. of NRI Patients")
...
statedetails <- read.csv("/Users/Ishneet Kaur Kataria/Downloads/DsProject-main/state_level_latest.csv")
statedetails
data = statedetails[,c(1,2,3,4,5)]

values <-  c(479779,279144,141531,158254 ,196789 )
states <-  c("Maharashtra","Tamil Nadu","Delhi","Karnataka", "Andhra Pradesh")

pie(values, labels = states, border = "red", lty = 2,main = "Top 5 states with maximum confirmed cases")
